...
void clockAPIBegin(ClockManager& mgr) {
    ...
    server->on("/clock/toggle", HTTP_POST, [](AsyncWebServerRequest* req) {
        ...
        logger.apif("[%s] POST /clock/toggle: enabled=%d", req->client()->remoteIP().toString().c_str(), (int)newEnabled);
    ...
    server->on("/clock/mode/toggle", HTTP_POST, [](AsyncWebServerRequest* req) {
        ...
        logger.apif("[%s] POST /clock/mode/toggle: mode=%s", req->client()->remoteIP().toString().c_str(), chimeModeToString(newMode));
